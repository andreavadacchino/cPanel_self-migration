...
| dns-uapi-encoding | **N1 co-bug — `+`/`%` corruption fix**: cpsrvd form-url-decodes uapi CLI arg VALUES (`+`→space, `%XX`→byte), silently corrupting every DKIM/SPF (base64 `+`) with status=1. `encodeUAPIArgValue` in `uapiArgsScript` (`%`→%25, `+`→%2B); only those two bytes change → zero regression. Test stub `dns_apply_cmd_test.go` made faithful (`unquote_plus`) = regression guard. Byte-verified live via the tool's exact write path. Gate: go-reviewer R1→R2 APPROVE PULITO, Docker LINUX_ALL_GREEN. See `DNS_MASS_EDIT_DIAGNOSIS_78.md` | #63 |
| dns-apex-dname | **N1 ROOT CAUSE — apex dname must be FQDN, not `@`**: `mass_edit_zone` rejects `dname="@"` (status=0 "The request failed", atomic on the whole batch); the tool sent `@` for the apex SPF replace via `dnsCanonToRelative`. Fix: apex → FQDN (`zone.`); 3 call sites inherit it; verify-after decoupled (`dnsCanonLiveName` handles live `@`). Bisected to a single add (`@`→status=0, `<zone>.`→status=1). Real `dns apply` post-fix: 3 applied/0 failed, verify CLEAN, DKIM/SPF source with `+` intact. Test `TestDNSCanonToRelativeApexUsesFQDN`. Gate: go-reviewer APPROVE PULITO, Docker LINUX_ALL_GREEN. Dogfooding #2 walk → `ready_for_cutover` (auto-transition). Verdict UI-only = **SÌ** (riserva N2). Full session report: `SESSION_20260705_N1_RESOLUTION.md` | #64 |
...
| webui-design-system | **Design system condiviso + landing entry unico (UI moderna)** (solo presentazione): nuovo `internal/webui/templates/_theme.html` con define `themeCSS` (design system professionale sobrio — token, card, tabelle, badge, bottoni, stepper, banner, danger zone, semafori) + `appHeader` (app bar brand+nav con stato attivo), parsato in ENTRAMBI i set di template (embed+ParseFS in `webui.go` e `workbench.go`). `/` da cruscotto tecnico a landing con hero+CTA al percorso guidato; `/workbench` e le 7 schermate con look coerente (stepper a pillole, call-out prossima azione, Conferme/Applica in card). Preservati id/campi form (`dns-apply-btn`, `dns-standalone-attest`, `migrate_content`, `confirm_account`, `action_key`), CSRF, glifi, verdetti, redazione segreti, traduzioni IT (#67). Motore/logica intatti (4 righe Go: embed+ParseFS). Validato: browser-walk 5 superfici sessione reale + go-reviewer APPROVE (1 LOW chiuso) + Docker LINUX_ALL_GREEN 20 ok/0 FAIL. Decisioni utente: scope «design system + entry unico», stile «professionale sobrio». `MANUAL_ACTIONS_IT_DESIGN.md` (traduzioni) | #68 |
| manual-actions-it | **Traduzione IT azioni manuali (Title/OperatorAction) — layer presentazione** (chiude l'ultimo residuo EN della UI): `manualTitleIT`/`manualActionIT` (funzioni pure, pattern `statusLabelIT`) registrate nel FuncMap dei due template set (dashboard #61 + workbench #66), chiamate al render. NON alla sorgente: la chiave acceptance = `sha256(type,section,title,detail)` → tradurre nel motore romperebbe gli `AK-*`; la UI legge l'artifact JSON congelato e ri-renderizza, così la sessione reale diventa IT senza rigenerare. Scope: Title+OperatorAction; `Detail` (diff valori) e `Type` restano grezzi; motore/`dnsplan.go` immutati. `webui.go` passa a `template.New("index.html").Funcs(...)` (trappola: Execute su root). Test: golden 29 title+31 operator, drift-guard anchor su `checklist.go` + noun-map bidirezionale, render end-to-end dashboard+Conferme. Validato coi DATI reali della sessione `mig_20260704` via HTTP: 6/6 azioni IT, zero EN, AK/Type verbatim. Gate: go-reviewer R1 APPROVE (LOW fixati)→R2 APPROVE PULITO; Docker LINUX_ALL_GREEN 20 ok/0 FAIL. `MANUAL_ACTIONS_IT_DESIGN.md` | #67 |
| workbench-ux | **Workbench UX Redesign v1 — percorso guidato in 7 schermate** (SOLO presentazione, enum motore byte-identici): Panoramica · Preflight · Fotografia · Cosa verrà migrato · Conferme · Applica · Chiusura, come GET sub-view additive non-breaking. `workbench_view.go` (read-only): `readArtifactFacts` fail-soft, `nextAction` mapping stato→azione totale su AllStatuses, `cutoverReadiness` (verdetto Chiusura da OverallStatus+blockers_cutover+conferme pendenti, NON dallo status forzabile), traduzione IT completa (status/step/overall/coverage note, 33 aree). `saveAcceptTo` (riuso accept, redirect parametrico). DNS Danger Zone: checkbox attestazione out-of-band disabilita Applica (client-side; enforcement reale = conferma forte). Gate: design doc+riesame opus (9 correzioni) → go-reviewer R1→R2 APPROVE PULITO → walk browser reale (dogfooding #3) → fix cosmetici IT → giro conferma APPROVE PULITO; Docker LINUX_ALL_GREEN 20 ok/0 FAIL. Limite dichiarato: contenuti azioni manuali restano EN (motore, come #61). `DOGFOODING_3_UX_WALK.md`, `PR_WORKBENCH_UX_REDESIGN_DESIGN.md` | #66 |
| flight-director-roadmap | **Roadmap frontend Flight Director (docs-only)** + spec dev-ready Job Journal: `FRONTEND_FLIGHT_DIRECTOR_ROADMAP.md` (workbench ingegneristica → cabina di regia migration-first, PR 69→75; §6 rehydration onesta, §7 events.jsonl solo migrate_content, §12 host.yaml escluso da bundle, §16 decisioni aperte) revisionata Opus con ispezione codice + corretta; `PR69_JOB_JOURNAL_DESIGN.md` spec del Job Journal. Nessun codice | #69 |
| job-journal | **In-Flight Job Rehydration Journal** (fondazione Flight Director; NON redesign): nuovo `job.json` per-working-dir (atomico 0600 come `store.writeSession`) con IDENTITÀ+fase dell'exec in corso/ultimo — schema **lean** ratificato (no item-level: riusato da `loadRunMonitor`; granularity opzione B, nessun writer toccato). Scoperta chiave: due livelli di rehydration già esistono (`readArtifactFacts` completato + `loadRunMonitor` item-level per migrate_content); il gap era solo lo strato IDENTITÀ dello slot anonimo. `handleExec`: running scritto PRIMA del subprocess, terminale nel defer con `release()`. **409 opaco eliminato** su tutti e 3 i chiamanti dello slot (`/run`/`/accept`/`/exec` via `writeBusy409`) — job.json scritto SOLO da `/exec` (scope boundary ratificato). Recovery `running`→`interrupted` allo startup (`recoverJobJournal`) + reconcile read-time nel view-model (no-write su GET). `readArtifactFacts` esteso con `areaFacts.BackupPresent` → Rollback offerto solo se backup esiste. `workbenchView.JobLive` + meta-refresh 2s in `wbHead` (riuso pattern dashboard) → 7 schermate live mentre running; interrupted/completed non refreshano (backstop execTimeout 30m). Anti-leak: solo identità+fase, mai credenziali/argv (testato anche sul failure path). Gate: go-reviewer R1 BLOCK (goroutine-leak nei test + gap anti-leak) → R2 APPROVE PULITO + APPROVE sul delta live-refresh; Docker LINUX_ALL_GREEN (test+vet=0), race webui+workbench verde; mini-review adversariale 12/12. `SESSION_20260706_PR70_JOB_JOURNAL.md`, `PR69_JOB_JOURNAL_DESIGN.md` | #70 |
| setup-wizard | **New Migration Wizard (setup flow)**: `/workbench/new` guida l'operatore (nome → sorgente → destinazione → account cPanel → cosa migrare → preflight). Modello: `Endpoint`/`ContentSelection`/`SetupMeta` + `Session.Setup *SetupMeta` (pointer omitempty → sessioni vecchie leggono); `Store.CreateWithSetup` (Create delega). **Credenziali metadata-only**: il wizard non raccoglie segreti (Endpoint senza campo segreto — guardia strutturale + canary su disco); `host.yaml` (0600) resta la sede delle credenziali via il form `/config` esistente, collegato da una callout. DNS **separato e opt-in** (dangerzone), nessun «migra tutto». La scelta contenuti **governa lo schermo Applica** (`contentScope`/`deriveContentScope` → `workbenchView.Scope`; aree escluse = «non incluso», niente apply/verify/rollback). **Gating frontend-only dichiarato**: `/exec` non gateato server-side, la conferma forte per-account resta il vero gate di scrittura. go-reviewer APPROVE ×2 (0 CRIT/HIGH); Docker LINUX_ALL_GREEN; walk live | #72 |
| scope-aware-next | **Next actions scope-aware** (chiude il debito UX di #72): il banner «prossima azione consigliata» ora cita SOLO le aree incluse in `Session.Setup.Content`. `contentScope.applyAreaLabels()`/`includedAreaLabels()`/`includedDetail()`; `nextAction(status,facts,scope)` e `missingVerifies(facts,scope)` filtrano per scope; `areaOrder` con predicato `inScope`. Preflight banner elenca le aree incluse; ReadyForApply cita solo le incluse; DNS incluso → nota prudente («area delicata»); DNS/Cron/EmailConfig esclusi → mai citati. **Legacy `Setup==nil` invariato** (scope = tutto incluso; testo apply storico byte-identico). Presentation-only: nessun writer/runner/apply/verify toccato. Docker LINUX_ALL_GREEN | #73 |
| flight-director-shell | **Flight Director UI shell** (da «schermate workbench» a cabina di regia — SOLO presentazione): header persistente della migrazione (nome, dominio, sorgente→destinazione con account, stato, **risk badge** onesto, tag DNS incluso/escluso, job in corso/interrotto, prossima azione), **timeline laterale** delle 7 fasi (stato Da fare/In corso/Fatto/Attenzione, fase corrente evidenziata, link alle route reali — nessuna route inventata) e **main stage** contestuale che riusa i template esistenti. Nuovo `workbench_flightdirector.go`: `buildRiskBadge`/`buildTimeline`/`applyPhaseState` (funzioni pure, derivano da status+artifact+job journal+scope; nessuna logica operativa). Ristrutturati `wbHead`/`wbFooter`, aggiunti `fdHeader`/`fdTimeline`, rimosso il vecchio pill-nav `wbNav`. Risk badge **non promette falso verde**: stati terminali (cutover/archiviata) vincono su checklist stale; job running/interrotto e blocker restano visibili. **Legacy `Setup==nil` invariato** (fallback source/destination profile, nessun tag DNS inventato); form `/exec` (migrate_content, conferma forte, DNS danger zone) intatti. **SSE ancora rimandata**. Gate: go-reviewer R1 REQUEST CHANGES (stati terminali + coerenza inventario) → R2 REQUEST CHANGES (regressione job-priority nel fix R1) → R3 APPROVE; Docker LINUX_ALL_GREEN; race webui+workbench verde | #75 |
| platform-migration-roadmap | **Platform Migration Roadmap (docs-only)**: piano di trasformazione tool→piattaforma smart. `PLATFORM_MIGRATION_ROADMAP.md` (15 sezioni). Scoperta chiave da ispezione codice: il motore è maturo/live-proven, il gap è **orchestrazione + contratto Plan→Scope→Execution**, non il motore. Tre decisioni prodotto bloccate (Opzione 1): orchestratore one-click con 1 sola conferma forte (DNS escluso dall'auto-run, riusa `pipelineSteps`+gate); DNS come task manuale verificabile (fotografa/classifica in 5 categorie, `dns_apply` resta Danger Zone avanzata, nessuna regressione); Migration Plan come read-model aggregato su artifact esistenti (riusa `readArtifactFacts`, no nuovo writer/CLI, `migration_plan.json` persistente rimandato). Roadmap a fasi (Fase 1…7, numerazione logica indipendente dai numeri GitHub). Nessun codice. Gate: `git diff --check` | docs |
| platform-migration-plan | **Fase 1 — Platform Migration Plan / Readiness** (prima PR di codice della roadmap): **read-model only** che risponde a «cosa succede se premo Avvia migrazione?». Nuovo `internal/webui/workbench_migration_plan.go`: `migrationPlan` + `buildMigrationPlan(f, scope)` puro/read-only/fail-soft, aggrega `artifactFacts` (via `readArtifactFacts`) + `contentScope`. 6 categorie (automatic/manual_verifiable/blocking_migration/blocking_cutover/informational/excluded). `CanStartMigration` = stesso oracolo di blocco di `nextAction` (`ApplyBlocked || OverallStatus==NOT_READY`, = gate reale `isApplyBlockedByChecklist`) **più** «almeno un'area automatica in scope» (l'orchestratore Fase 3 esegue solo aree automatiche) → più conservativo, mai contraddice il blocco. Blocker di aree escluse mostrati a parte (`ExcludedBlockers`), mai nascosti (gate globale, sezioni globali/ignote fail-open). **DNS sempre manuale/verificabile, MAI auto-runnable** (dns_apply resta avanzato); Cron/EmailConfig automatici solo se il piano esiste. UI: `screen_migrazione` col blocco «Piano migrazione»; CTA one-click **disabilitata** («disponibile nella Fase 3»). **Nessun** nuovo writer/CLI, **nessun** `migration_plan.json` persistente (deferred), `/exec`+strong-confirm immutati, `contentScope` non gate server-side. Test: 10 unit + 2 render. Gate: gofmt/vet puliti, go test webui/workbench/config verde, race verde, Docker LINUX_ALL_GREEN; go-reviewer R1 REQUEST CHANGES → R2 APPROVE | #78 |
| scope-confirmation | **Fase 2 — Scope Confirmation after Preflight**: usa il Migration Plan (Fase 1) per far confermare/raffinare all'operatore cosa migrare dopo il preflight. `internal/webui/workbench_scope_confirm.go`: `presetContent` (all_safe/site/email/files/databases/custom; **DNS mai automatico**, checkbox indipendente manuale/verificabile), `hasAutomaticArea` (DNS-only NON conta), `canEditScope(f,jobLive)` (scope congelato dopo `report.json`/`<area>_apply_report.json` o job live), `handleConfirmScope` (POST `/workbench/session/<id>/scope`, CSRF; rifiuta DNS-only → `?scope=need_area`; success → `?scope=updated`). `internal/workbench`: `SetupMeta.ScopeConfirmedAt *time.Time` (omitempty, backward-compat) + `Store.ConfirmScope` (mutazione METADATA, timeline `scope_confirmed`, legacy Setup nil acquisisce Setup). UI `screen_migrazione`: blocco «Conferma cosa vuoi migrare» + flash + **CTA state-aware** `migrationCTALabel` (non-pronto/bloccato/non-confermato/confermato-pronto), bottone resta disabilitato. **Non toccati**: strong-confirm/isApplyBlockedByChecklist/actionRegistry/pipelineSteps; `contentScope` NON gate server-side (Fase 3); nessun writer/CLI nuovo. Test: 11 webui + 2 store. Gate: gofmt/vet puliti, go test verde, race verde, Docker LINUX_ALL_GREEN; go-reviewer | Fase 2 |
| smart-orchestrator | **Fase 3 — Smart Migration Orchestrator**: una sola strong-confirmation avvia in sequenza le aree **automatiche/safe/in-scope**, stop-on-first-failure, DNS **mai** nell'auto-run. Nuovo `internal/webui/workbench_orchestrator.go`: `buildOrchestratorPhases(dir,f,scope)` deriva le fasi server-side (contenuti se File/DB/Email in scope → un solo `migrate_content` con `--file/--db/--mail` coerenti; config email/cron **solo se il piano esiste**, come classifica il Migration Plan), `runOrchestration` esegue con **gate `isApplyBlockedByChecklist` ricontrollato PRIMA di ogni fase write** (§14.3), verify inline dove esiste (email/cron con **`--fail-on-drift`** così un drift ferma il run), `migrate_content` = `completed_with_report` (nessun verify clean finto), attach artifact best-effort, journal per-fase. `handleStartMigration` (POST `/workbench/session/<id>/start-migration`, CSRF): richiede Setup + `ScopeConfirmedAt`, **una** strong-confirmation, ricalcola `artifactFacts`+`contentScope`+`MigrationPlan` server-side, rifiuta se `!CanStartMigration`/nessuna fase automatica, riserva lo slot single-writer condiviso, redirect `?migrate=<code>`. **`contentScope` diventa gate server-side reale** per l'orchestratore (area esclusa = nessun flag/fase). Estratti helper argv condivisi in `workbench_exec.go` (`migrateContentArgv`, `email/cronApplyArgv`, `email/cronVerifyArgv(failOnDrift)`) riusati sia dal registry `/exec` sia dall'orchestratore → **nessuna duplicazione di logica pericolosa, nessun nuovo writer/CLI**. UI: bottone «Avvia migrazione» attivo (`StartEnabled`) solo con piano pronto+scope confermato+nessun job live; `migrationCTALabel(p,jobLive)` aggiornata; flash `migrateFlash` (done/done_manual/partial/gate_stopped/blocked/no_auto/scope_unconfirmed/needs_setup). **Nessun** rollback automatico, **nessun** `migration_plan.json`, **nessuna** SSE, `dns_apply` resta azione avanzata. Test: 18 webui orchestratore (15 obbligatori + gate mid-run §14.3 + CSRF + cron-senza-piano) + regressione `/exec`/plan/scope verdi. Gate: gofmt/vet puliti, go test webui/workbench/config verde, race verde; go-reviewer + Docker LINUX_ALL_GREEN (gate utente) | Fase 3 |
