...
| 6D | **DNS apply writer — the ONLY DNS writer** (closes the DNS track): `MassEditZoneAdd`/`MassEditZoneRemove` via `RunUAPI` with indexed param format (`add-0=<JSON>`, `remove-0=<int>` — 6D-pre fact 1); `ExtractSOASerial` (base64 decode from parse_zone SOA); `IsStaleSerialError` detection (exact string from 6D-pre fact 3); `FetchDNSZoneRaw` for backup. First DNS allowlist in `dns_safety_test.go` + `TestDNSWriteAllowlistFilesExist` guard + `mass_edit_zone` in checklist writeCalls. cpapi2 fixed via CageFS disable (root session: cause was CageFS isolation, not jailshell; `cagefsctl --disable giorginisposi`). Smoke debt reconciled: cron/filter write-path NOT achievable with giorginisposi (0 cron, 0 filtri on source); SetMXCheck primitive verified via root CLI but tool codepath untested (routing skip on this account). 6D-pre in `PR6D_PRE_CAPTURES.md` (add/remove round-trip, stale-serial error, non-propagation, peer standalone). Gate: go-reviewer R1 (0 HIGH, 2 MEDIUM → fixed); Docker LINUX_ALL_GREEN | #52 |
