# Task D2: Deep content verification
...
| **Size** | L |
| **Dependencies** | D1 |
| **Branch** | `feat/d2-deep-content-verification` |

**Goal:** Add bounded manifests/digests and semantic database/mail checks with explicit unavailable states and performance budgets.

**Current State:** The V2 has no byte/content verification for files, databases, or messages.

```text
apps/api/app/modules/readiness/engine.py
```
...
- [ ] same counts with different content fails; unverifiable is never clean; large accounts use bounded strategies.
